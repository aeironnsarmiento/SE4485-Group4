...
layout = [
    ...
    [sg.Text('Zip File:'), sg.InputText('', key='ZIP_FILE'), sg.FileBrowse(file_types=(("Zip Files", "*.zip"),))],
    [sg.Button('Package VSCode Env'), sg.Button('Select Zip File to Unpackage'), sg.Button('Setup VSCode Env')]
]
...
while True:
    ...
    elif event == 'Select Zip File to Unpackage':
        zip_file_path = sg.popup_get_file('Select your VSCode zip file to unpackage', file_types=(("Zip Files", "*.zip"),))
        if zip_file_path:
            window['ZIP_FILE'].update(zip_file_path)
